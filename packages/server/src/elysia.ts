...
const contextHandler = async ({
  headers,
  ip,
}: { headers: Record<string, string>; ip: string }): Promise<Context> => {
  const auth = headers.authorization;
  if (!auth)
    return { user: null, unauthorizedError: 'No auth token provided', db: db };
...
export const createContext = new Elysia()
  .derive(async ({ headers }) => {
    ...
    if (!auth)
      return {
        user: null,
        unauthorizedError: 'No auth token provided',
        db: db,
      };
